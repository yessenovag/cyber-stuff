...
function FeedbackModal({ onClose, token }: { onClose: () => void; token: string | null }) {
  ...
  const submitFeedback = async () => {
    if (!rating) return;
    try {
      await fetch("https://cybersafe-api-v3-dzf7cdd7czewged8.spaincentral-01.azurewebsites.net/api/feedback", {
        method: "POST",
        headers: {
...
export default function AIChat() {
  ...
  useEffect(() => {
    ...
    fetch("https://cybersafe-api-v3-dzf7cdd7czewged8.spaincentral-01.azurewebsites.net/api/chats", {
      ...
      .then(async (dbChats: { id: number; title: string }[]) => {
        if (dbChats.length === 0) {
          // Create first chat
          const res = await fetch("https://cybersafe-api-v3-dzf7cdd7czewged8.spaincentral-01.azurewebsites.net/api/chats", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const loadMessages = async (chatId: number): Promise<Message[]> => {
    ...
    try {
      const res = await fetch(`https://cybersafe-api-v3-dzf7cdd7czewged8.spaincentral-01.azurewebsites.net/api/chats/${chatId}/messages`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const sendMessage = async (text?: string) => {
    ...
    if (token) {
      fetch(`https://cybersafe-api-v3-dzf7cdd7czewged8.spaincentral-01.azurewebsites.net/api/chats/${activeChatId}/messages`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      // Rename chat in DB
      if (newTitle) {
        fetch(`https://cybersafe-api-v3-dzf7cdd7czewged8.spaincentral-01.azurewebsites.net/api/chats/${activeChatId}/title`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
    ...
    try {
      const res = await fetch("https://cybersafe-api-v3-dzf7cdd7czewged8.spaincentral-01.azurewebsites.net/api/ai/chat", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(token && { Authorization: `Bearer ${token}` }),
  ...
  const deleteChat = async (chatId: number, e: React.MouseEvent) => {
    ...
    if (token) {
      await fetch(`https://cybersafe-api-v3-dzf7cdd7czewged8.spaincentral-01.azurewebsites.net/api/chats/${chatId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
    }
